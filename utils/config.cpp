...
const QString &Config::getConfigPath()
{
    if (s_configPath.isEmpty()) {
        s_configPath = QString::fromLocal8Bit(qgetenv("UDB_CONFIG_PATH"));
...
void Config::setUserBootConfig(const UserBootConfig &config)
{
    ...
    m_userData->setValue(COMMON_BITRATE_INDEX_KEY, config.bitRate);
    m_userData->setValue(COMMON_MAX_SIZE_INDEX_KEY, config.maxSize);
...
UserBootConfig Config::getUserBootConfig()
{
    ...
    config.bitRate = m_userData->value(COMMON_BITRATE_INDEX_KEY, COMMON_BITRATE_INDEX_DEF).toUInt();
    config.maxSize = m_userData->value(COMMON_MAX_SIZE_INDEX_KEY, COMMON_MAX_SIZE_INDEX_DEF).toUInt();
